...
interface PracticeRecordDao {
    ...
    suspend fun getAllPracticeRecordsSync(): List<PracticeRecord>

    @Query("SELECT * FROM practice_records WHERE problemId IN (SELECT id FROM problems WHERE unitId IN (SELECT id FROM study_units WHERE projectId = :projectId))")
    suspend fun getPracticeRecordsForProjectSync(projectId: Int): List<PracticeRecord>
    ...
    @Query("DELETE FROM practice_records WHERE problemId IN (SELECT id FROM problems WHERE unitId IN (SELECT id FROM study_units WHERE projectId = :projectId))")
    suspend fun deleteRecordsForProject(projectId: Int)

    @Query("DELETE FROM practice_records")
    suspend fun deleteAllPracticeRecords()
}
